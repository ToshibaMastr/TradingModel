...
class TradeDataset(Dataset):
    def __init__(
        ...
        self.input = self.df[["close", "high", "low", "volume"]]
        self.target = self.df[["target", "target", "target", "target"]]
    ...
    def __getitem__(self, idx):
        ...
        input_seq = self.input.iloc[s_begin:s_end]
        target_seq = self.target.iloc[r_begin:r_end]

        price, mn, mx = scale(input_seq[["close", "high", "low"]])
        volume, _, _ = scale(input_seq[["volume"]])
        target, _, _ = scale(target_seq, mn, mx)
